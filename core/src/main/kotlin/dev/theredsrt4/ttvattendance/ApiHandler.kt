...
abstract class ApiHandler(protected val clientID: String, private val clientSecret: String, fileName: String) {
    ...
    init {
        println("Checking for AUTH File...")
        authFile.parentFile?.mkdirs()
        if(!authFile.exists()) {
            println("AUTH File does not exist, creating...")
            println("Creating AUTH File...")
            ...
            try {
                println("AUTH File FOUND! Reading...")
                // read from file
